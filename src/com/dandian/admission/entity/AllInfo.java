...
public class AllInfo {
	...
	private Dictionary studentTab; // 学生信息�?
	...
	private String curriculums;// �?带课�?
	private String classesStr;// �?带班�?
	private String companyName;// 单位名称
	private String workAttendances;// 考勤名称
	private String workAttendanceValues;// 考勤分�??
	...
	public AllInfo(JSONObject jo) {
		...
		workAttendanceValues = jo.optString("考勤分�??");
		...
		Log.d(TAG, "------------------>jo:" + jo);
		Log.d(TAG, "------------------>�?始准备初始化�?" + new Date());
		classesStr = jo.optString("�?带班�?");
		Log.d(TAG, "------------------>�?带班级：" + classesStr);
		if (classesStr != null) {
		...
		JSONObject joStuInfo = jo.optJSONObject("学生详情信息�?");
		Log.d(TAG, "------------------>学生详情信息卡：" + joStuInfo);
		studentTab = new Dictionary();
		studentTab.setParentCode("studentTab");
		studentTab.setParentName("学生详情信息�?");
		studentTab.setItemCode("学生详情信息�?");
		if (joStuInfo != null) {
	...
	public AllInfo(net.minidev.json.JSONObject jo) {
		curriculums = (jo.get("�?带课�?")==null?"":jo.get("�?带课�?").toString());
		companyName = jo.get("单位名称").toString();
		workAttendances = jo.get("考勤名称").toString();
		workAttendanceValues = jo.get("考勤分�??").toString();
		...
		maxWeek = Integer.parseInt(jo.get("�?大周�?").toString());
		...
		PrefUtility.put(
		 ...
		 PrefUtility.put(Constants.PREF_MAX_WEEK, maxWeek);
		 PrefUtility.put(Constants.PREF_CUR_XUEQI, curXueQi);
		classesStr = (jo.get("�?带班�?")==null?"":jo.get("�?带班�?").toString());
		Log.d(TAG, "------------------>�?带班级：" + classesStr);
		if (classesStr != null) {
			String[] classes = classesStr.split(",");
			studentList = new HashMap<String,List<Student>>();
		...
		if (jotest != null) {
			...
			for (int i = 0; i < jotest.size(); i++) {
				...
				TestStartEntity temp = new TestStartEntity(item.get("名称").toString(),Integer.parseInt(item.get("�?").toString()));
				startTestList.add(temp);
			}
		}
		...
		net.minidev.json.JSONObject joStuInfo = (net.minidev.json.JSONObject)jo.get("学生详情信息�?");
		Log.d(TAG, "------------------>学生详情信息卡：" + joStuInfo);
		studentTab = new Dictionary();
		studentTab.setParentCode("studentTab");
		studentTab.setParentName("学生详情信息�?");
